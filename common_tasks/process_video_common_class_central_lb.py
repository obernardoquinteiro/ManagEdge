...
class VideoProcessor:
    def __init__(self, connection):
        self.frames = []
        self.imbalancesListen = []
        self.imbalances30 = []
        self.imbalances60 = []
        self.imbalances120 = []
        self.averageImbalance: float = 1
        self.connection = connection

    def calculateClusterImbalance(self, imbalanceIndex, imbalanceDict):
        import os
        import multiprocessing
        lock = multiprocessing.Lock()
        clusterImbalance: int = 0
        self.imbalancesListen.append(imbalanceIndex)
        if len(self.imbalancesListen) == MAX_SIZE_LISTEN_LIST + 1:
            self.imbalancesListen.pop(0)
            sum: int = 0
            for x in self.imbalancesListen:
                sum += x
            self.averageImbalance = float(sum/MAX_SIZE_LISTEN_LIST)
            sum = 0
            try:
                with lock:
                    imbalanceDict[int(os.getpid())] = self.averageImbalance
                    for key in imbalanceDict.keys():
                        sum += imbalanceDict[key]

                        clusterImbalance = float(sum/len(imbalanceDict))
                        
            except Exception as e:
                print(f"Error imbalanceDict: {e}")
    ...
    def process_video(self, imbalanceDict):
        import debugpy
        import os
        import sys
        import time
            ...
            while True:
                ...
                try:
                    ...
                    if setup == Setup.ALL_EDGE.value:
                        ...
                        serverImbalance: int = self.calculateClusterImbalance(imbalanceIndex, imbalanceDict)
                        data_to_send: Dict[float] = {
                            "imbalanceIndex": float(imbalanceIndex),
                            "serverImbalance": float(serverImbalance)
                        }
    ...
    def run(self, imbalanceDict):
        try:
            print("Run")
            self.process_video(imbalanceDict)
        except KeyboardInterrupt:
            print("Keyboard interrupt. Processing frames and creating video.")
        except Exception as e:
            print(f"Error: {e}")
...
def start_video_processing(connection, imbalanceDict):
    # Instantiate the VideoProcessor class with the provided arguments
    processor = VideoProcessor(connection)
    # Run the video processing logic
    processor.run(imbalanceDict)
