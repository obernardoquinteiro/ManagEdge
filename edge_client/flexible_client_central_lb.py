...
def client_init(cap: cv2.VideoCapture, frame_size: int, net: cv2.dnn.Net, ln: Sequence[str], encoder: np.ndarray[np.float32], tracker: Tracker, movement_data_writer: csv.writer, crowd_data_writer: csv.writer, host: str, port: int, currentSetup: int):
	try:
		#client.bind((clientHost, clientPort))
...
def checkForImbalance(currentSetupDict, lock, serverImbalance):
	global listenList
	...
	if currentSetupDict[0] == Setup.ALL_EDGE.value:
		if serverImbalance<=MIN_THRESHOLD_CENTRAL_IMBALANCE and imbalanceIndex <= serverImbalance:
			with lock:
				print(serverImbalance)
				print("Change 1 Server")
				currentSetupDict[0] = Setup.HALF_EDGE_SERVER.value
			listenList = []
		elif imbalanceIndex<=MIN_THRESHOLD_CAMSERVER_IMBALANCE:
			with lock:
		...
		if serverImbalance<=MIN_THRESHOLD_CENTRAL_IMBALANCE and imbalanceIndex <= serverImbalance:
			...
			with lock:
				...
				currentSetupDict[0]  = Setup.ALL_SERVER.value
			listenList = []
		elif serverImbalance>=MAX_THRESHOLD_CENTRAL_IMBALANCE and imbalanceIndex >= serverImbalance:
			with lock:
				print(serverImbalance)
				print("Change 0 Central")
		...
		if serverImbalance>=MAX_THRESHOLD_CENTRAL_IMBALANCE and imbalanceIndex >= serverImbalance:
			with lock:
				print(imbalanceIndex)
				print("Change 1 Central")
				currentSetupDict[0] = Setup.HALF_EDGE_SERVER.value
			listenList = []
		elif imbalanceIndex>=MAX_THRESHOLD_CAMSERVER_IMBALANCE:
			with lock:
				...
				currentSetupDict[0] = Setup.HALF_EDGE_SERVER.value
			listenList = []
	elif currentSetupDict[0] == Setup.BACK_SERVER.value:
		if serverImbalance<=MIN_THRESHOLD_CENTRAL_IMBALANCE and imbalanceIndex <= serverImbalance:
			with lock:
				print(imbalanceIndex)
				print("Change 5 Central")
...
def listenToServer(currentSetupDict, lock):
	...
	while True:	
		...
		serverImbalance:float = received_data["serverImbalance"]
		if len(listenList) == MAX_SIZE_LISTEN_LIST:
			listenList.pop(0)
			listenList.append(imbalanceIndex)
			checkForImbalance(currentSetupDict, lock, serverImbalance)
		else:
			listenList.append(imbalanceIndex)
...
def main() -> None:
	...
	parser.add_argument('--port', type=int, default=None, help='Port number to connect to')
